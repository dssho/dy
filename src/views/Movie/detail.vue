<template>
    ...
    <div v-else id="content" class="contentDetail">
      ...
      <div class="detail_player swiper-container">
       ...
       <div class="detail_list">
        <div class="detail_list_bg" :style="{ 'background-image' : 'url('+ detailMovie.images.medium.replace(/w\.h/,'148.208') +')' }"></div>
        <div class="detail_list_filter"></div>
        <div class="detail_list_content">
          <div class="detail_list_img">
            <img :src="detailMovie.images.medium | setWH('148.208')" alt="">
          </div>
          <div class="detail_list_info">
            <h2>{{ detailMovie.title }}</h2>
            <p><span>国家:  </span>{{ detailMovie.countries[0] }}</p>
            <p><span>年份:  </span>{{ detailMovie.year }}</p>
            <p><span>语言:  </span>{{ detailMovie.languages[0] }}</p>
            <p><span>时长:  </span>{{ detailMovie.durations[0] }}</p>
            <!-- <p v-for="(item,index) in detailMovie.tags" :key="index">{{ item }}</p> -->
            <p> 类型:
               <span v-for="i in  detailMovie.genres " :key="i.id">{{i}}  </span>
             </p>
          </div>
        </div>
      </div>
      ...
      <div class="detail_player swiper-container" ref="detail_player">
        <ul class="swiper-wrapper">
          <li v-for="(item,index) in detailMovie.photos" :key="index" class="swiper-slide">
            <div>
              <img :src="item.image " style="width:80px;height:67px;" alt=""> 
